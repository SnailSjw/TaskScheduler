...
每个任务关联独立的日志表 (`task_logs`)，通过外键 `CASCADE` 与任务表关联。日志按 **天分组** 以卡片形式展示，每天卡片顶部显示日期、星期和执行结果标签（成功/失败）。
...
应用支持 **Light / Dark** 双主题，跟随系统设置自动切换。浅色模式使用 `#F2F3F8` 浅灰蓝背景，深色模式使用 `#0C0E14` 深邃黑背景。功能色在两套主题中分别调校以确保对比度和可读性。
